...
function statusFor(data: JsonRecord, section: string): JsonRecord {
  return record(record(data.section_status)[section]);
}

function sectionOk(data: JsonRecord, section: string): boolean {
  return statusFor(data, section).status !== "error";
}

function sectionError(doc: Document, section: string, data: JsonRecord): void {
...
function clear(node: HTMLElement | null): void {
  while (node?.firstChild) node.removeChild(node.firstChild);
}

function cell(doc: Document, row: HTMLElement, value: unknown, code = false): void {
...
function card(doc: Document, label: string, value: unknown): HTMLElement {
...
function setVisible(doc: Document, id: string, visible: boolean): void {
  const node = doc.getElementById(id);
  if (node) node.hidden = !visible;
}

export function renderAdminDashboard(doc: Document, raw: unknown): void {
  ...
  if (sectionOk(data, "overview")) {
    ...
    for (const [label, content] of cards) overview?.appendChild(card(doc, label, content));
  ...
  if (sectionOk(data, "devices")) {
    ...
    for (const item of rows) {
      ...
      for (const [value, code] of values) cell(doc, row, value, Boolean(code));
