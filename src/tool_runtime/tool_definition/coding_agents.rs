...
use super::{
    def, model_spec, permission_risk, ToolDefinition, PERMISSION_RISK_JOB,
    ...
    permission_risk(
        model_spec(
            def(
                ...
                // The input carries an explicit Project, but false here is
                // intentional: Workflow current-session fallback must never
                // synthesize recording_session_id or Run authority.
                false,
                NoPath,
                true,
                false,
