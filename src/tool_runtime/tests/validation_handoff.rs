...
async fn long_cargo_test_hands_off_to_queryable_job() {
    ...
    let log = runtime
        .job_log_for_auth(job_id.clone(), None, Some(200), None)
        .await;
    assert!(log.success);
...
async fn partial_agent_job_logs_null_complete_validation_counts_everywhere() {
    ...
    let log = runtime
        .job_log_for_auth(job_id.clone(), None, Some(200), Some(&auth))
        .await;
...
async fn local_job_status_uses_bounded_logs_and_nulls_complete_counts() {
    ...
    let record = crate::tool_runtime::local_jobs::LocalJobRecord::new("demo".to_string(), dir);
