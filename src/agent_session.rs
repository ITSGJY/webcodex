...
/// the writer is joined, and the disconnect is reconciled so running jobs are
/// marked lost and the client decays to stale/offline.
///
    ...
    match env {
        AgentEnvelope::Result { payload } => {
            ...
            if let Err(e) = registry
                .complete_for_connection(payload.clone(), connection_id)
                .await
            {
                tracing::warn!(
                    client_id = client_id,
        ...
        AgentEnvelope::JobUpdate { payload } => {
            if let Err(e) = registry
                .update_job_for_connection(payload.clone(), connection_id)
                .await
            {
                tracing::warn!(
                    client_id = client_id,
        ...
        AgentEnvelope::Ping { ts } => {
            ...
            if let Err(e) = out_tx.try_send(AgentEnvelope::Pong { ts: *ts }) {
                let reason = match e {
