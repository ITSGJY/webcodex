...
当前 release candidate 已通过完整 Rust binary suite（main 1,750 passed / 4 ignored，CLI 220 passed，runner 402 passed / 2 ignored）、focused process-group cleanup tests、源码与 release checks、前端 typecheck/test/dist 检查以及 npm self-test。WebSocket 与 polling zero-config E2E 均为 108/108，coding-loop compare 为 6/6；79 个已跟踪 Markdown 文件中的 438 个本地链接全部有效；release-mode npm package smoke 也成功临时安装并运行三个 0.3.0 binaries。
