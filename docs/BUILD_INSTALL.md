...
A `win32-x64` npm install becomes officially published only in the release commit that carries a real Windows-host-built artifact (`webcodex-v<VERSION>-win32-x64.tar.gz`, built on a Windows release host with `scripts/package_release_artifact.ps1` from the exact tag) and its real checksum in `manifest.json`. Until then the manifest keeps no `win32-x64` entry. The Windows development/release gate is the `test-windows` CI lane plus `npm --prefix npm/webcodex test` and `scripts/npm_install_windows_smoke.ps1` on native Windows.
