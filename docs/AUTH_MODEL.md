...
The fixed `mcp:bridge` permission follows the same opt-in rule for managed-user
PAT/OAuth access and is also excluded from direct shared-key, shared-key OAuth,
open-anonymous, and project-share authority. A Runner advertising the
`mcp_bridge` capability is only an availability fact; it never grants the
scope or changes the principal's existing Runner/project/shared-key binding.
OAuth access is additionally audience-bound to the complete canonical
`/mcp/bridge/<bridge_id>` resource. The authorization code and both token kinds
preserve that exact RFC 8707 resource, and request-time authentication rejects
ordinary `/mcp` or different-bridge audiences before the fixed scope and live
Runner/provider identity checks. Non-OAuth first-party credentials retain their
existing scope and ownership behavior.
