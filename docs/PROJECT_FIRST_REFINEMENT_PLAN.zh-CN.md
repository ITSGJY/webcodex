...
| `validation_recipe_not_found` | auto resolution 从 `cwd` 到 Task root 没有 supported marker | false | true | 提供含 marker 的 `cwd`；仅 test 可显式选择 manifestless Python unittest |
...
- 同一最近目录有多个 marker 时 auto 返回 `validation_recipe_ambiguous`；显式
  recipe 只能解除该目录内真实 marker 的歧义；唯一例外是显式 Python test 在未
  选中 `pyproject.toml` 时以 `cwd` 为 root 使用固定 unittest plan；
- Rust 保留 Cargo 三项行为和 safe one-argv test filter；Node 从有效
  `packageManager` 或单一 lockfile 解析 manager，只选择固定 script 名顺序；
  Python 的 Ruff/Black、Ruff/Mypy、pytest 只由 `pyproject.toml` 配置证明，
  manifestless Python 只支持 `python -B -m unittest discover -v`；Go 只支持
  `go vet ./...` 与 `go test ./...`，format 明确 unavailable；
