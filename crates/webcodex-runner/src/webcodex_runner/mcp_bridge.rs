//! Persistent Runner-owned stdio MCP providers for the hosted MCP bridge.
//!
...
use super::config::{McpBridgeConfig, McpBridgeProviderConfig};
use crate::mcp_bridge::{
    validate_json_value, validate_request, validate_tool_result, validate_tools, McpBridgeContent,
    McpBridgeDispatchState, McpBridgeProvider, McpBridgeRequest, McpBridgeResponse,
    McpBridgeResponsePayload, McpBridgeTool, McpBridgeToolResult, MCP_BRIDGE_MAX_MESSAGE_BYTES,
};
...
struct ProviderEntry {
    config: McpBridgeProviderConfig,
    instance_id: String,
    failed: AtomicU8,
...
impl McpBridgeManager {
    pub(crate) fn new(config: &McpBridgeConfig) -> Self {
...
impl ProviderEntry {
    fn advertisement(&self) -> McpBridgeProvider {
        McpBridgeProvider {
...
impl ProviderConnection {
    fn spawn(config: &McpBridgeProviderConfig, timeout: Duration) -> Result<Self, ProviderFailure> {
        ...
        let initialized = connection.request(
            ...
            json!({
                ...
                "capabilities": {},
                "clientInfo": {
                    "name": "webcodex-runner-mcp-bridge",
    ...
    fn tools_list(&mut self, timeout: Duration) -> Result<Vec<McpBridgeTool>, ProviderFailure> {
    ...
    fn tools_call(
        ...
        arguments: Value,
        timeout: Duration,
    ) -> Result<McpBridgeToolResult, ProviderFailure> {
        ...
        let result = McpBridgeToolResult {
            content,
...
fn read_bounded_line(reader: &mut impl BufRead) -> Result<Option<Vec<u8>>, ReaderFault> {
    ...
    let read = (&mut *reader)
        .take((MCP_BRIDGE_MAX_MESSAGE_BYTES + 2) as u64)
        .read_until(b'\n', &mut line)
    ...
    if line.last() != Some(&b'\n') {
        return Err(if line.len() > MCP_BRIDGE_MAX_MESSAGE_BYTES {
    ...
    if line.is_empty() || line.len() > MCP_BRIDGE_MAX_MESSAGE_BYTES {
        return Err(if line.len() > MCP_BRIDGE_MAX_MESSAGE_BYTES {
            ReaderFault::TooLarge
        } else {
            ReaderFault::Malformed
...
fn bridge_error(
    state: McpBridgeDispatchState,
    code: &'static str,
    message: &'static str,
) -> McpBridgeResponse {
    McpBridgeResponse::error(state, code, message)
}

fn stale_provider() -> McpBridgeResponse {
    bridge_error(
        McpBridgeDispatchState::NotStarted,
        "stale_provider",
