...
#[test]
fn mcp_bridge_register_request_projects_bounded_provider_inventory_without_local_launch_details() {
    let tmp = tempfile::tempdir().unwrap();
    let mut cfg = test_config(tmp.path().join("config/projects.d"));
    cfg.mcp_bridge.providers = vec![webcodex_runner::config::McpBridgeProviderConfig {
