...
pub const MCP_BRIDGE_MAX_PROVIDERS: usize = 8;
pub const MCP_BRIDGE_MAX_PROVIDER_ID_BYTES: usize = 64;
pub const MCP_BRIDGE_MAX_PROVIDER_NAME_BYTES: usize = 128;
pub const MCP_BRIDGE_MAX_TOOL_COUNT: usize = 128;
pub const MCP_BRIDGE_MAX_TOOL_NAME_BYTES: usize = 128;
pub const MCP_BRIDGE_MAX_DESCRIPTION_BYTES: usize = 4 * 1024;
pub const MCP_BRIDGE_MAX_SCHEMA_BYTES: usize = 64 * 1024;
pub const MCP_BRIDGE_MAX_ARGUMENT_BYTES: usize = 64 * 1024;
pub const MCP_BRIDGE_MAX_STRUCTURED_CONTENT_BYTES: usize = 128 * 1024;
pub const MCP_BRIDGE_MAX_TEXT_CONTENT_BYTES: usize = 64 * 1024;
pub const MCP_BRIDGE_MAX_RESULT_BYTES: usize = 256 * 1024;
pub const MCP_BRIDGE_MAX_CONTENT_ITEMS: usize = 32;
pub const MCP_BRIDGE_MAX_MESSAGE_BYTES: usize = 1024 * 1024;
pub const MCP_BRIDGE_MAX_JSON_DEPTH: usize = 16;
pub const MCP_BRIDGE_MAX_JSON_NODES: usize = 4_096;
pub const MCP_BRIDGE_MAX_JSON_STRING_BYTES: usize = 64 * 1024;
...
pub enum McpBridgeDispatchState {
    NotStarted,
    OutcomeUnknown,
    Completed,
}
...
pub enum McpBridgeContent {
    Text { text: String },
}

#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct McpBridgeToolResult {
    pub content: Vec<McpBridgeContent>,
    #[serde(
        default,
...
pub struct McpBridgeResponse {
    pub dispatch_state: McpBridgeDispatchState,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub payload: Option<McpBridgeResponsePayload>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub error: Option<McpBridgeError>,
}

impl McpBridgeResponse {
    pub fn success(payload: McpBridgeResponsePayload) -> Self {
        Self {
...
pub fn validate_provider_id(value: &str) -> Result<(), String> {
    validate_identifier(value, "provider_id", MCP_BRIDGE_MAX_PROVIDER_ID_BYTES, true)
}
...
pub fn validate_provider_instance_id(value: &str) -> Result<(), String> {
    validate_identifier(
        ...
        MCP_BRIDGE_MAX_PROVIDER_ID_BYTES,
        false,
    )
}

pub fn validate_tool_name(value: &str) -> Result<(), String> {
    validate_identifier(value, "tool name", MCP_BRIDGE_MAX_TOOL_NAME_BYTES, false)
}
...
pub fn validate_provider_name(value: &str) -> Result<(), String> {
    if value.trim().is_empty() || value.len() > MCP_BRIDGE_MAX_PROVIDER_NAME_BYTES {
        return Err(format!(
            "provider name must contain 1..={} bytes",
            MCP_BRIDGE_MAX_PROVIDER_NAME_BYTES
        ));
    }
...
pub fn validate_request(request: &McpBridgeRequest) -> Result<(), String> {
    match request {
        ...
        McpBridgeRequest::ToolsCall {
            ...
            validate_json_value(arguments, MCP_BRIDGE_MAX_ARGUMENT_BYTES, "tool arguments")
        }
    }
}

pub fn validate_response(response: &McpBridgeResponse) -> Result<(), String> {
    let encoded = serde_json::to_vec(response)
        .map_err(|_| "bridge response could not be serialized".to_string())?;
    if encoded.len() > MCP_BRIDGE_MAX_MESSAGE_BYTES {
        return Err(format!(
    ...
    match (&response.payload, &response.error) {
        ...
        (Some(_), None) if response.dispatch_state != McpBridgeDispatchState::Completed => {
...
fn validate_error(error: &McpBridgeError) -> Result<(), String> {
...
pub fn validate_providers(providers: &[McpBridgeProvider]) -> Result<(), String> {
    if providers.len() > MCP_BRIDGE_MAX_PROVIDERS {
        return Err(format!(
...
pub fn validate_tools(tools: &[McpBridgeTool]) -> Result<(), String> {
    if tools.len() > MCP_BRIDGE_MAX_TOOL_COUNT {
        return Err(format!(
            "tool count exceeds maximum {}",
            MCP_BRIDGE_MAX_TOOL_COUNT
        ));
    }
    ...
    for tool in tools {
        ...
        if let Some(description) = tool.description.as_deref() {
            if description.len() > MCP_BRIDGE_MAX_DESCRIPTION_BYTES {
                return Err(format!(
                    "tool description exceeds maximum {} bytes",
                    MCP_BRIDGE_MAX_DESCRIPTION_BYTES
                ));
            }
        ...
        if !tool.input_schema.is_object() {
            return Err(format!(
                "inputSchema for tool '{}' must be a JSON object",
                tool.name
            ));
        }
        validate_json_value(
            &tool.input_schema,
            MCP_BRIDGE_MAX_SCHEMA_BYTES,
            "tool inputSchema",
        )?;
    }
    Ok(())
...
pub fn validate_tool_result(result: &McpBridgeToolResult) -> Result<(), String> {
    if result.content.len() > MCP_BRIDGE_MAX_CONTENT_ITEMS {
        return Err(format!(
            "tool result content exceeds maximum {} items",
            MCP_BRIDGE_MAX_CONTENT_ITEMS
        ));
    }
    let mut text_bytes = 0usize;
    for content in &result.content {
        match content {
            McpBridgeContent::Text { text } => {
                if text.len() > MCP_BRIDGE_MAX_TEXT_CONTENT_BYTES {
                    return Err(format!(
                        "tool result text exceeds maximum {} bytes",
                        MCP_BRIDGE_MAX_TEXT_CONTENT_BYTES
                    ));
                }
    ...
    if let Some(structured) = result.structured_content.as_ref() {
        ...
        validate_json_value(
            structured,
            MCP_BRIDGE_MAX_STRUCTURED_CONTENT_BYTES,
            "structured tool result",
        )?;
    }
    let encoded = serde_json::to_vec(result)
        .map_err(|_| "tool result could not be serialized".to_string())?;
    if encoded.len() > MCP_BRIDGE_MAX_RESULT_BYTES || text_bytes > MCP_BRIDGE_MAX_RESULT_BYTES {
        return Err(format!(
            "tool result exceeds maximum {} bytes",
            MCP_BRIDGE_MAX_RESULT_BYTES
        ));
    }
    Ok(())
}
...
fn validate_json_node(
    ...
    if depth > MCP_BRIDGE_MAX_JSON_DEPTH {
        return Err(format!(
            "{field} exceeds maximum JSON depth {}",
            MCP_BRIDGE_MAX_JSON_DEPTH
        ));
    }
    *nodes = nodes.saturating_add(1);
    if *nodes > MCP_BRIDGE_MAX_JSON_NODES {
        return Err(format!(
            "{field} exceeds maximum JSON node count {}",
            MCP_BRIDGE_MAX_JSON_NODES
        ));
    }
    match value {
        Value::String(text) => {
            if text.len() > MCP_BRIDGE_MAX_JSON_STRING_BYTES {
                return Err(format!(
                    "{field} contains a string larger than {} bytes",
                    MCP_BRIDGE_MAX_JSON_STRING_BYTES
                ));
            }
        }
        Value::Array(values) => {
...
mod tests {
    ...
    fn rejects_recursive_and_excessive_values() {
        let mut value = json!({});
        for _ in 0..=MCP_BRIDGE_MAX_JSON_DEPTH {
            value = json!({"next": value});
        }
        assert!(
            validate_json_value(&value, MCP_BRIDGE_MAX_SCHEMA_BYTES, "schema")
                .unwrap_err()
                .contains("depth")
        );

        let arguments = json!({"value": "x".repeat(MCP_BRIDGE_MAX_ARGUMENT_BYTES)});
        assert!(
            validate_json_value(&arguments, MCP_BRIDGE_MAX_ARGUMENT_BYTES, "arguments").is_err()
        );
    }

    #[test]
    fn rejects_duplicate_tools_and_binary_content() {
        let tool = McpBridgeTool {
            name: "echo".to_string(),
            description: None,
        ...
        assert!(serde_json::from_value::<McpBridgeToolResult>(raw).is_err());
    }
}
